...
- **Backgrounds** — a painted `Assets/VN/Resources/VN/Backgrounds/<name>.png` is used when
  ...
  Every scene the story uses is painted: `fog seawall beach_dusk hallway classroom rooftop
  town_night sea_gate bunker inn_room title`. Only `white` and `black` stay procedural,
  which is what they are for.

  These plates were generated by the `vn_backgrounds` pipeline from a Kanamori Bay scene
  set added to its `SCENES` list (the `k-` entries), on the same checkpoint as the
  character sprites so the composite has no style seam. An earlier attempt reused that
  project's original "Ashenveil" plates — an old European town — which fought the cast
  badly: a pastel cherry-blossom bridge behind white powered armour, a manor hall standing
  in for a school corridor. Regenerate or re-roll any scene with:
